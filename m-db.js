...
(function (preact) {
  ...
  const libSites = [
    {
      ...
      querys: {
        panelQueryStr:
          ".video-meta-panel>.columns.is-desktop .panel.movie-panel-info",
        codeQueryStr: `[data-clipboard-text]`,
      },
      method() {
        const columnVideoCover = document.querySelector(".column-video-cover");
        if (columnVideoCover && window.innerWidth > JUDGE_MOBILE_WIDTH) {
  ...
  const siteList = [
    ...
    },
    {
      ...
      domQuery: {
        // 标签区的第一个一般是字幕标签
        subQuery:
          '.space-y-2 a.text-nord13[href="https://missav123.com/chinese-subtitle"]',
        // 有个「切換無碼」按钮，藏在分享按钮旁边……
        leakQuery: ".order-first div.rounded-md a[href]:last-child",
      },
    },
    ...
    },
    {
      ...
      codeFormater: (preCode) =>
        preCode.startsWith("MIUM") ? `${SP_PREFIX}${preCode}` : preCode,
    },
    {
      name: "JavDB",
      hostname: "javdb.com",
    ...
    return (
      n &&
        (n.__c &&
          ...
          (n.__c.__H = null)),
        null != (n = g({}, n)).__c &&
          (n.__c.__P === e2 && (n.__c.__P = t2), (n.__c = null)),
        (n.__k =
          n.__k &&
  ...
  (preact.options.unmount = function (n) {
    ...
    (L.prototype.render = function (n, e2) {
      ...
      var i2 =
        e2.__a && preact.createElement(preact.Fragment, null, n.fallback);
      return (
        i2 && (i2.__u &= -33),
        [
      ...
      (function (n2) {
        ...
        "select" == e2 &&
          ...
          t2.class && !t2.className
            ? ((u2.class = t2.class),
              Object.defineProperty(u2, "className", tn))
  ...
  function serachPageParser(
    ...
    const isSuccess =
      ...
      titleNode &&
      matchCode &&
      isCaseInsensitiveEqual(matchCode[0], CODE);
    if (!isSuccess) {
      return {
        isSuccess: false,
      };
    }
    const targetLinkText = linkNode.href.replace(
      linkNode.hostname,
      siteHostName
    );
    return {
  ...
  const SiteBtn = ({ siteItem, CODE, multipleNavi, hiddenError }) => {
    ...
    const resultLink =
      (multipleFlag
        ...
        : fetchRes == null
        ? void 0
        : fetchRes.targetLink) ?? originLink;
  ...
  const App = x(function ({ libItem, CODE }) {
    ...
    const [multipleNavi, setMultipleNavi] = p(
      _GM_getValue("multipleNavi", true)
    );
    const [hiddenError, setHiddenError] = p(_GM_getValue("hiddenError", false));
    const list = siteList.filter(
      (siteItem) =>
        !disables.includes(siteItem.name) &&
        !siteItem.hostname.includes(libItem.name)
    );
    return u(preact.Fragment, {
      children: [
        u("div", {
  ...
  function main() {
    const libItem = libSites.find((item) =>
      item.href.test(window.location.href)
    );
    if (!libItem) {
